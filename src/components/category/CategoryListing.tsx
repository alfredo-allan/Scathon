...
import { CategoryFilters, type ColorOption, type PriceBucket, type RatingOption } from './CategoryFilters'
...
export function CategoryListing({ category, products }: CategoryListingProps) {
  ...
  const filteredProducts = useMemo(() => {
    return products.filter((product) => {
      ...
      if (selectedColors.size > 0 && !product.colors.some((color) => selectedColors.has(color.name))) {
        return false
      }
      if (
        selectedPriceBuckets.size > 0 &&
        !PRICE_BUCKETS.some((bucket) => selectedPriceBuckets.has(bucket.id) && bucket.test(product.price))
      ) {
        return false
      }
      return true
  ...
  const hasActiveFilters = selectedColors.size > 0 || selectedPriceBuckets.size > 0 || minRating !== null || onlyNew
    ...
    <div className="px-4 md:px-8 py-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 border-b border-neutral-200 dark:border-neutral-800 pb-4">
        <h1 className="text-xl font-semibold tracking-tight text-neutral-950 dark:text-neutral-50 md:text-2xl">
          {category.label} <span className="text-base font-normal text-neutral-500 dark:text-neutral-400">({sortedProducts.length})</span>
        </h1>
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="flex items-center gap-2 rounded-app border border-neutral-300 dark:border-neutral-700 px-4 py-2 text-xs font-medium uppercase tracking-widest text-neutral-800 dark:text-neutral-100 lg:hidden">
            Filtros
            {hasActiveFilters && <span className="h-1.5 w-1.5 rounded-full bg-neutral-900 dark:bg-neutral-100" aria-hidden />}
          </button>
        ...
        <div>
          {sortedProducts.length === 0 ? (
            <div className="flex flex-col items-center gap-3 py-24 text-center">
              <p className="text-sm text-neutral-600 dark:text-neutral-400">Nenhum produto encontrado com esses filtros.</p>
              {hasActiveFilters && (
                <button
                  type="button"
      ...
      <aside
        ...
        <div className="border-t border-neutral-200 dark:border-neutral-800 p-4">
          <button
            type="button"
            onClick={() => setMobileFiltersOpen(false)}
            className="w-full rounded-app bg-neutral-950 py-3 text-xs font-semibold uppercase tracking-widest text-neutral-50 dark:bg-neutral-100 dark:text-neutral-950">
